...
def main():
    ...
    run(a.binary,'init',str(directory),'example.test.')
    ...
    run('openssl','req','-new','-newkey','ec','-pkeyopt','ec_paramgen_curve:P-256','-nodes','-keyout',str(directory/'mail.key'),'-out',str(directory/'mail.csr'),'-subj','/CN=mail-good.example.test')
    (directory/'mail.ext').write_text('basicConstraints=critical,CA:FALSE\nkeyUsage=critical,digitalSignature\nextendedKeyUsage=serverAuth\nsubjectAltName=DNS:mail-good.example.test,DNS:mail-bad.example.test\n')
    ...
    for flavour in ['good','bad']:
        mail=f'mail-{flavour}.example.test.'
        records += [record(f'{flavour}.example.test.','Mx',dict(preference=10,exchange=mail)),record(mail,'A','127.0.0.1'),record(mail,'Aaaa','::1')]
    ...
    for i in range(240):records.append(record(f'bulk-{i:04}.example.test.','Txt',[list((f'load-{i:04}:'+('x'*160)).encode())]))
    # Names exercise closest encloser and wildcard paths under external validation.
    records += [record('leaf.branch.example.test.','A','192.0.2.10'),record('*.wild.example.test.','A','192.0.2.11')]
    (directory/'config.json').write_text(json.dumps(config,indent=2)+'\n')
    (directory/'fixture-summary.json').write_text(json.dumps({'tlsa_sha256':digest.hex(),'records':len(records),'zone':'example.test.','signature_validity':600,'refresh_before':300},indent=2)+'\n')
