...
root=pathlib.Path('/work');master=(root/'container-ready').read_text().strip() if (root/'container-ready').exists() else socket.gethostbyname('host.docker.internal');name=(TRANSFER_KEY_NAME)
keys=dns.tsigkeyring.from_text({name:base64.b64encode((root/'tsig.key').read_bytes()).decode()})
query=dns.message.make_query((VALIDATION_DOMAIN + '.'),'SOA');query.use_tsig(keys,name,algorithm='hmac-sha256')
response=dns.query.udp(query,master,port=18535,timeout=3);assert response.had_tsig
current=response.answer[0][0].serial
messages=list(dns.query.xfr(master,(VALIDATION_DOMAIN + '.'),rdtype='IXFR',serial=current-1,port=18535,keyring=keys,keyname=name,keyalgorithm='hmac-sha256',relativize=False,timeout=3,lifetime=10))
assert len(messages)>1 and all(m.had_tsig for m in messages)
zone=dns.zone.from_xfr(iter(messages),relativize=False);assert zone.get_rdataset((VALIDATION_DOMAIN + '.'),'SOA')[0].serial==current
same=list(dns.query.xfr(master,(VALIDATION_DOMAIN + '.'),rdtype='IXFR',serial=current,port=18535,keyring=keys,keyname=name,keyalgorithm='hmac-sha256',relativize=False,timeout=3,lifetime=10))
