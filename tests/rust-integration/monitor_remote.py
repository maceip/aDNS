...
while True:
 now=int(time.time());answer=dns.query.udp(dns.message.make_query('example.test.','SOA',want_dnssec=True),a.server,port=a.port,timeout=5)
 ...
 for name,kind in [('example.test.','SOA'),('definitely-absent-agentdns.example.test.','A')]:
  result=subprocess.run(['delv','@'+a.server,'-p',str(a.port),'-a',str(a.anchor),'+root=example.test.',name,kind],capture_output=True,text=True,timeout=15)
