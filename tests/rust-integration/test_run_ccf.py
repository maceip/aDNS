...
class RunnerGuardTests(unittest.TestCase):
    ...
    def test_snapshot_is_exact_allowlist_and_survives_source_mutation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            registry=root/'registry.json';registry.write_text('{"fixture":true}')
            expected['.domain-registry/topology.json']=hashlib.sha256(registry.read_bytes()).hexdigest()
            with mock.patch.object(runner,'REPOSITORY',root),mock.patch.object(runner,'registry_path',return_value=registry):actual=runner.snapshot_sources(snapshot)
            ...
            self.assertEqual({str(p.relative_to(snapshot)) for p in snapshot.rglob('*') if p.is_file()},set(expected))
