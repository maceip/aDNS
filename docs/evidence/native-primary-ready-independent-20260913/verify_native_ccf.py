#!/usr/bin/env python3
"""Offline, independent verification of an actual raw CCF SNP-v3/v5 node quote.
...
SPEC_V3 = 'https://docs.amd.com/api/khub/documents/x_tYkgJNtfpvqwPq45NrSQ/content'
MAX_INPUT = 1024 * 1024
...
def layout(report):
    ...
    version = int.from_bytes(report[:4], 'little')
    require(version in (3, 5), 'this independent verifier supports only report versions 3 and 5')
    reserved_start = 520 if version == 5 else 504
    for begin, end in [(76, 80), (395, 416), (491, 492), (495, 496),
                       (reserved_start, 672), (720, 744), (792, 1184)]:
    ...
    return {'version': version, 'signed_region': {'start': 0, 'end_inclusive': 671},
            'launch_mitigation_vector': u64(report, 504) if version == 5 else None,
            'current_mitigation_vector': u64(report, 512) if version == 5 else None,
            'mitigation_vector_policy': ('authenticated inventory; supplied policy has no mitigation-bit floor' if version == 5
                                         else 'not present in report version 3; bytes504..671 are reserved'),
...
def verify(quote_raw, peer_raw, policy_raw, ark_pem, now):
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name, offset in [('report_data', 80), ('report_data_padding', 112), ('measurement', 144), ('cce', 192),
                             ('launch_mitigation_vector' if fields['version'] == 5 else 'reserved_504', 504),
                             ('current_mitigation_vector' if fields['version'] == 5 else 'reserved_512', 512), ('reserved_520', 520)]:
    ...
    result = {'status': 'passed', 'purpose': 'independent-offline-ccf-node-quote-cross-check-only', 'verification_time': now,
              'specification': {'url': SPEC if fields['version'] == 5 else SPEC_V3,
                                'layout': 'AMD report v5' if fields['version'] == 5 else 'AMD report v3', 'table': 23},
